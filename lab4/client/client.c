...
int main (int argc, char *argv[])
{
	...
	while (fileACK == 0)
	{
		...
		if (rv == 0) //no ACK received after 2 seconds. timeout
		{
			if (ACKcounter < 3)
			{
				ACKcounter++;
			...
			{
				printf("Timed out 3 times waiting for ACK. Terminating...\n");
				return 1;
			}
		}

		else if (rv == 1) //ACK was received. no timeout
		{
			ACKcounter = 0;
			//wait for ACK
			PACKET ACKorNAK;
	...
	while(!feof(src))
	{
		...
		switch(seq)
		{
			case 0:
				...
				if (rv == 0) //no data going to socket. timeout
				{
					if (ACKcounter < 3)
					{
						ACKcounter++;
